-- 040_finance_cash_correctifs — deux défauts mesurés sur 039, corrigés.
--
-- ADDITIF PUR. 039 est APPLIQUÉE : on ne la retouche pas (règle 9). Les deux
-- défauts ci-dessous ont été mesurés en interrogeant la porte sur la base réelle
-- juste après son application ; ils sont corrigés ici par `CREATE OR REPLACE`.
--
-- ═══ DÉFAUT 1 — le libellé de mois sortait en anglais ═════════════════════
-- `to_char(date, 'TMMon')` rend le mois dans la locale `lc_time` DU SERVEUR.
-- Mesuré sur cette instance : « Aug ». L'interface est intégralement en
-- français (ADR-008, CLAUDE.md §4) et une locale de serveur n'est pas un
-- contrat — elle peut changer sans que le dépôt bouge. Table de correspondance
-- explicite, donc déterministe.
--
-- ═══ DÉFAUT 2 — `plus_ancien_impaye_jours` rendait `null` ═════════════════
-- `MAX(...)` sur un ensemble vide rend NULL. Sans impayé, l'écran recevait
-- `null` là où le contrat de la porte dit « zéro ligne rend des zéros, jamais
-- null ». Distinct de `panier_moyen`, qui reste NULL À DESSEIN : un panier
-- moyen sans séance n'existe pas, alors qu'un âge d'impayé sans impayé vaut
-- bien zéro jour.
--
-- 🔴 AUCUN `DROP` — un DROP emporterait le propriétaire `app_gatekeeper` et la
-- porte reviendrait à `postgres` (rolbypassrls). `CREATE OR REPLACE` + `ALTER
-- FUNCTION … OWNER TO` immédiatement après.

BEGIN;

GRANT CREATE ON SCHEMA app TO app_gatekeeper;

-- Le corps est celui de 039 §2, à deux expressions près — signalées par
-- « 040 » en commentaire à l'endroit exact du changement.
...
INSERT INTO app.schema_migrations (version) VALUES ('040_finance_cash_correctifs')
