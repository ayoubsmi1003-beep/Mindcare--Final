-- ═══════════════════════════════════════════════════════════════════════════
-- 057_workspace_echelles_id_fix — max(uuid) n'existe pas.
--
-- TROUVÉ PAR LE CHECKPOINT, PAS EN RELICTURE. Premier appel de
-- get_patient_workspace après 056 sur un dossier porteur de mesures :
-- `function max(uuid) does not exist`. L'agrégat d'id ajouté par 056
-- utilisait `max()`, qui n'est défini ni sur uuid ni sur la plupart des
-- types non ordonnables.
--
-- CORRECTIF MINIMAL : `(array_agg(r.admin_id) FILTER (WHERE r.rang = 1))[1]`.
-- Avec le tri interne `ORDER BY r.administered_at DESC` du row_number(), le
-- premier élément de l'agrégat EST l'administration la plus récente du rang.
-- Une seule ligne change ; le reste du corps est recopié depuis pg_proc
-- (méthode 048/052/055).
--
-- Retour arrière (documentation) : réapplication du corps 056. Humain.
-- ═══════════════════════════════════════════════════════════════════════════
...
(array_agg(r.admin_id ORDER BY r.rang))[1] AS d_id,
...
COMMENT ON FUNCTION app.get_patient_workspace(uuid) IS
  'Patients V3 (057). Correctif 056 : l''id de dernière administration '
  's''obtient par array_agg(...)[1] — max() n''existe pas sur uuid. Contrat '
  'inchangé sinon.';
...
INSERT INTO app.schema_migrations (version) VALUES ('057_workspace_echelles_id_fix')
