-- 081_fix_080_regression_and_workspace_treatments_v2 — 080 a régressé
-- `derniere_consultation` en la rebasant sur 076 au lieu de 078.
--
-- ADDITIF PUR. 078 est APPLIQUÉE : on ne la retouche pas (règle 9).
--
-- ═══ CE QUI S'EST PASSÉ ═══════════════════════════════════════════════════
-- 080 corrigeait un vrai défaut (traitements_v2.en_pause/arretes_recents
-- incomplets, cf. son propre en-tête) mais l'a construit en repartant du
-- CORPS DE 076, pas du corps réellement en base. Or 078 avait DÉJÀ corrigé
-- `derniere_consultation` : `app.consultations` ne porte pas de colonne
-- `kind` — 076 lisait `c.kind` (qui n'a jamais existé, bug hérité de 047),
-- 078 lit `a.kind` via une jointure sur `app.appointments`. En redéployant le
-- corps de 076, 080 a réintroduit `c.kind` et cassé `get_patient_workspace`
-- pour TOUT patient (erreur `42703 column c.kind does not exist`),
-- immédiatement constaté en base après application de 080.
--
-- Cette migration repart du corps RÉEL de 078 (vérifié par lecture directe du
-- fichier, pas supposé) et y applique SEULEMENT le correctif de 080 :
-- `en_pause`/`arretes_recents` rendent le même jeu de colonnes que `actifs`.
--
...
INSERT INTO app.schema_migrations (version) VALUES ('081_fix_080_regression_and_workspace_treatments_v2')
