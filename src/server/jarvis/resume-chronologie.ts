...
import { DOMAINES_SOURCE, type SourceRef } from "./resume-cas";
...
/**
 * `Array.isArray()` RÉTRÉCIT VERS `any[]`, PAS VERS `unknown[]` — c'est une
 * particularité de la bibliothèque standard de TypeScript, et elle laisse
 * ensuite passer n'importe quel accès de membre sans vérification. Dans du code
 * qui décide ce qui est CITABLE dans un résumé clinique, c'est exactement ce
 * qu'on ne veut pas.
 *
 * Ce garde rend `readonly unknown[]` : chaque élément doit ensuite être vérifié
 * par `estObjet()`. Le lint du dépôt (`no-unsafe-member-access`) l'exigeait
 * déjà ; `supabase/functions/**` étant hors de sa portée, personne ne l'a vu.
 */
function estTableau(v: unknown): v is readonly unknown[] {
...
export function construireApercu(socle: unknown): Apercu | null {
  ...
  if (Array.isArray(socle["echelles"])) {
    for (const e of socle["echelles"]) {
      ...
      const points: readonly unknown[] = estTableau(e["points"]) ? e["points"] : [];
